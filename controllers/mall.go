...
func (this *MallBaseController) ProductQuery() {
	...
	if id==0 {
		...
		sql = ps("select p.*,p.start_date<%d as presale,pt.name from product as p,product_type as pt where p.pt_id=pt.id and p.id='%d';", TimeNow , id)
	}
...
func (this *MallController) ProductPayDetailsCdb() {

	pdid := this.GetString("pdid")
	start_date := this.GetString("start_date")
	end_date := this.GetString("end_date")
	// 检查参数
	if !CheckArg(pdid,start_date,end_date) {
		this.Rec = &Recv{5, "参数存在空值", nil}
		return
	}

	// 查询所有商品名称信息
	sql := ps("select `friendpdt_no` from `user_product` where product_no=%s;", pdid)
	var result []orm.Params
	db := orm.NewOrm()
	nums, err := db.Raw(sql).Values(&result)
	if err != nil {
		log("该设备不存在:[%v：%v]", pdid, err)
		this.Rec = &Recv{5, ps("该产品不存在"), nil}
		return
	} else if nums <= 0 {
		this.Rec = &Recv{5, ps("该产品不存在"), nil}
		return
	...
	alias := result[0]["friendpdt_no"].(string)

	cd, da := PayDetails(alias, start_date, end_date)
	if cd != 200 {
		this.Rec = &Recv{5, ps("查询失败"), nil}
		return
	} else {
		this.Rec = &Recv{3, ps("查询成功"), &da}
		return
	}
}
