...
func (this *AdminController) QueryUser() {
	...
	var sql, sqlc string = "", ""
	sql = ps("SELECT id,account,nick,intro,active,wallet,src,city,realname,idnumber,unix,log_unix from `user` where flag>0 order by unix desc limit %d,%d;", begidx, counts)
...
// sid,begidx,counts,flag
func (this *AdminController) FlagQueryUser() {
	begidx, _ := this.GetInt32("begidx")
	counts, _ := this.GetInt32("counts")
	...
	var sql, sqlc string = "", ""
	sql = ps("SELECT id,account,nick,intro,active,wallet,src,city,realname,idnumber,unix,log_unix from `user` where flag=%d order by unix desc limit %d,%d;", flag, begidx, counts)
	sqlc = ps("SELECT count(id) AS num from `user` where flag=%d;", flag)
...
func (this *AdminController) DelUser() {
	// 身份判断
	if this.User.Flag != 1 {
		this.Rec = &Recv{5, ps("仅平台管理员可调用"), nil}
		return
	}
	...
	// 检查是权限
	var sql string = ps("SELECT supid from `user` where id='%d';", id)
	var account string = ""
	db := orm.NewOrm()
	var result []orm.Params
	_, err := db.Raw(sql).Values(&result)
	if err != nil {
		...
		if len(result) > 0 {
			...
			if int64(supid) > 0 {
				this.Rec = &Recv{5, "无权删除不属于本平台的用户.", nil}
				return
			}
		}
	}
...
func (this *AdminController) SpecifyUser() {
	...
	var sql string = "SELECT id,account,nick,intro,level,flag,wallet,src,city,log_unix from `user` where flag=0"
	if account != "" {
		if strings.Contains(sql, "where") {
			sql += ps(" and account='%s'", account)
		} else {
	...
	if bdt > 0 {
		if strings.Contains(sql, "where") {
			sql += ps(" and unix>=%d", bdt)
		} else {
			sql += ps(" where unix>=%d", bdt)
		}
	}
	if edt > 0 {
		if strings.Contains(sql, "where") {
			sql += ps(" and unix<%d", edt)
		} else {
			sql += ps(" where unix<%d", edt)
		}
	}
...
func (this *AdminController) UserAuthInfoBkQuery() {
	...
	sql := ps("select id,realname,idnumber,positive_img,negative_img,verify_status,verify_reason,bankcard from user where account='%s';", account)
	var result []orm.Params
