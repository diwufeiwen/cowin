...
func (this *ManufactController) ManufactOrderUndone() {
	...
	// 查询所有商品名称信息
	sql := ps("select id from `product` where pt_id=%d and status=1;", this.User.Ptid)
	var result []orm.Params
	db := orm.NewOrm()
	...
	sql = ps("select id,product_name,start_date,end_date,coverurl from `product` where pt_id=%d and status=1 limit %d,%d;", this.User.Ptid, begidx, counts)
	log("%s", sql)
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询产品失败:[%v]", err)
...
func (this *ManufactController) ManufactOrderHistory() {
	...
	sql = ps("select id,product_name,start_date,end_date from `product` where pt_id=%d and status=1 limit %d,%d;", this.User.Ptid, begidx, counts)
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询产品失败:[%v]", err)
		this.Rec = &Recv{5, ps("查询产品失败"), nil}
