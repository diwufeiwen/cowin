...
func (this *ShopcartController) ShopcartQuery() {
	sql := ps("SELECT sc.*,p.product_name,p.original_price,p.discount_price,p.imgurl,p.coverurl,p.web_intro,p.app_intro,p.update_unix,p.unix,p.start_date<%d as presale from `shopping_cart` as sc,`product` as p where p.id=sc.pid and sc.user_id='%d';", TimeNow, this.User.UserId)
...
func (this *ShopcartController) ShopcartSettle() {
	...
	// 查询购物单中产品信息
	var sql string = ps("select * from `shopping_cart` where id=%d;", id)
	var result []orm.Params
	db := orm.NewOrm()
	nums, err := db.Raw(sql).Values(&result)
	if err != nil {
