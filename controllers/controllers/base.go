...
func (this *BaseController) Prepare() {
	...
	// log("alipay: %v", this.Ctx.Request)
	if url != "/cowin/alipay/notice" {
		var keys []string
		...
		if _, ok := this.Ctx.Request.Form["ver"]; !ok {
			this.Rec = &Recv{5, "版本号未传", "no data"}
			goto EXIT_BASEPREPARE
		}

		if _, ok := this.Ctx.Request.Form["ts"]; !ok {
			this.Rec = &Recv{5, "必传参数不全", "no data"}
			goto EXIT_BASEPREPARE
		} else {
			...
			if ts-3*60 > int(TimeNow) || ts+3*60 < int(TimeNow) {
				...
				this.Rec = &Recv{5, "请校准本地时间", "no data"}
				goto EXIT_BASEPREPARE
			}
		}
	...
	if sid == "" {
		...
		if TimeNow-this.User.LastTime > SidLife {
			...
			if _, ok = this.User.Auth[url]; ok {
				this.User.LastTime = TimeNow //更新最新通信时间
				if this.User.Flag > 0 {
					this.Rec = &Recv{5, "权限不够,请检查", "no data"}
				}
			} else {
...
func (this *BaseController) Finish() {
	...
	if this.Rec.Code != 6 && this.Rec.Msg != "签名错误" {
		this.ServeJSON()
	}
}
