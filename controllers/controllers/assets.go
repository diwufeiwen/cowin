...
func (this *AssetsController) AssetsProducting() {
	...
	// 业务逻辑
	var sql, sqlc string
	sql = "SELECT ep.*,u.account,u.realname from `enjoy_product` as ep,`user` as u where ep.user_id=u.id and ep.status<4 and ep.pay_status=1"
	...
	sql += ps(" order by ep.desc limit %d,%d;", begidx, counts)
	sqlc += ";"
...
func (this *AssetsController) AssetsPutin() {
	...
	sql += ps(" order by up.desc limit %d,%d;", begidx, counts)
	sqlc += ";"
	...
	if data.Total > 0 {
		...
		for idx := range result {
			...
			if err != nil {
				...
				item["growth_fans_num"] = res[0]["growth_fans_num"].(string)
				item["use_num"] = res[0]["use_num"].(string)
			}
		}
	}
