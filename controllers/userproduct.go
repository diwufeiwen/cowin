...
// ex_no(外部编号)
func (this *UserProductBaseController) ProductOperations() {
	...
	sql := ps("SELECT id,user_id from `user_product` where `friendpdt_no`='%s';", ex_no)
	db := orm.NewOrm()
	var result []orm.Params
	...
	sql = ps("update `user_product` set status='4',unix='%d' where id=%d;", TimeNow, id)
	...
	// 添加通知消息
	str := ps("你的编号为[%d]的产品已投入运营,请去查看", id)
	...
	// 添加物流信息
	sql = ps("SELECT count(up.id) as cnts,ep.id FROM user_product as up,enjoy_product as ep where up.ep_id=ep.id AND up.`status`<4 and up.`status`>-1 and up.ep_id=(SELECT ep_id FROM user_product WHERE friendpdt_no='%s');", ex_no)
	log("%s", sql)
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询未投入运营托管产品数量失败:[%v]", err)
	} else {
		cnts, _ := strconv.Atoi(result[0]["cnts"].(string))
		if cnts == 0 {
			ep_id, _ := strconv.Atoi(result[0]["id"].(string))

			// 添加物流信息
			AddLogisticsInfo(0, int64(ep_id), "已投入运营,请去查看投放中产品...")
		}
	}

	this.Rec = &Recv{3, "投入运营成功", nil}
	return
}
...
func (this *UserProductBaseController) ProductSerialMatch() {
	...
	sql := ps("SELECT id,user_id from `user_product` where `product_no`='%s';", product_no)
	db := orm.NewOrm()
	var result []orm.Params
	nums, err := db.Raw(sql).Values(&result)
	if err != nil {
	...
	// 添加通知消息
	str := ps("你的编号为[%s]的产品已匹配单号,请前去查看", product_no)
	sql = ps("insert into `letter` (title,content,send_uid,recv_uid,unix) values ('%s','%s','0','%d','%d')",
		"通知", str, user_id, TimeNow)
	_, err = db.Raw(sql).Exec()
	if err != nil {
...
func (this *UserProductController) UserProductUseRecord() {
	...
	// 业务逻辑
	sql := ps("SELECT * from `product_use` where up_id=%d order by unix desc;", id)
	db := orm.NewOrm()
	var result []orm.Params
	_, err := db.Raw(sql).Values(&result)
	if err != nil {
...
func (this *UserProductController) UserProductGeneral() {
	...
	// 查询自己在该城市运营产品总数
	sql := ps("select count(up.id) as opts from `user_product` as up,`enjoy_product` as ep where ep.id=up.ep_id and ep.pt_id=%d and ep.hosted_city like '%%%s%%' and up.user_id=%d and up.status=4;", pt_id, city, this.User.UserId)
	db := orm.NewOrm()
	var result []orm.Params
	_, err := db.Raw(sql).Values(&result)
	if err != nil {
	...
	// 查询该城市运营产品总数
	sql = ps("select count(up.id) as pts from `user_product` as up,`enjoy_product` as ep where ep.id=up.ep_id and ep.pt_id=%d and ep.hosted_city like '%%%s%%'and up.status=4;", pt_id, city)
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询总数出错:[%v]", err)
		this.Rec = &Recv{5, "查询总数失败", nil}
	...
	sql = ps("select sum(i.owner_income) as yincome from `user_product` as up,`enjoy_product` as ep,`income` as i where  ep.id=up.ep_id and up.id=i.up_id and up.status=4 and ep.pt_id=%d and ep.hosted_city like '%%%s%%' and i.unix>=%d and i.unix<%d;", pt_id, city, today_t.AddDate(0, 0, -1).Unix(), today_t.Unix())
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询总数出错:[%v]", err)
		this.Rec = &Recv{5, "查询总数失败", nil}
	...
	// 总收益
	sql = ps("select sum(i.owner_income) as income from `user_product` as up,`enjoy_product` as ep,`income` as i where ep.id=up.ep_id and up.id=i.up_id and up.status=4 and ep.pt_id=%d and ep.hosted_city like '%%%s%%';", pt_id, city)
	_, err = db.Raw(sql).Values(&result)
	if err != nil {
		log("查询总数出错:[%v]", err)
		this.Rec = &Recv{5, "查询总数失败", nil}
...
func (this *UserProductController) UserProductQuery() {
	...
	sql := ps("select count(up.id) as totals from `user_product` as up,`enjoy_product` as ep where ep.id=up.ep_id and ep.pt_id=%d and up.user_id=%d and up.status=4", pt_id, this.User.UserId)
	if city != "" {
	...
	for idx := range result {
		...
		//查询累计收益
		sql = ps("select sum(i.owner_income) as income from `user_product` as up,`income` as i where up.id=i.up_id and up.status=4 and up.id='%s';", item["id"].(string))
		_, err = db.Raw(sql).Values(&res)
		if err != nil {
			log("查询总数出错:[%v]", err)
			this.Rec = &Recv{5, "查询总数失败", nil}
...
func (this *UserProductController) UserPdtPickReceipt() {
	...
	// 更新订单包含产品状态为已收货
	sql = ps("update `user_product` set `status`=2 where id in (select up_id from `userpdt_pickup` where id=%d);", id)
	_, err = db.Raw(sql).Exec()
	if err != nil {
